...
3. Einmalig den Schlüsseltausch als dediziertem Service-Account auslösen: `docker compose exec -u pull_user server /usr/local/bin/push_ssh_key.sh`.
...
Der Server-Container bringt einen eigenen Linux-Account (`pull_user`) mit, der beim Build via `USERNAME`/`USER_PASSWORD` überschrieben werden kann (`docker compose build --build-arg USERNAME=my_user --build-arg USER_PASSWORD=...`). Führe `push_ssh_key.sh` und `pull_restic_repo.sh` immer mit `docker compose exec -u <USERNAME> server …` aus, damit SSH-Schlüssel und `known_hosts` konsistent im Home dieses Accounts landen.
